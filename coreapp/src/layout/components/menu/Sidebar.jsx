...
export default function Sidebar({ visible, setVisible }) {
    const customise = useSelector(state => state.customise); // Redux state access

    const [collapsed, setCollapsed] = useState(false); // State for managing sidebar collapse

    // Effect to handle sidebar collapse based on Redux state
    useEffect(() => {
        setCollapsed(!!customise.sidebarCollapsed);
    }, [customise.sidebarCollapsed]);

    // Function to close the sidebar, used in child components
    const onClose = () => {
        setVisible(false);
    };
        ...
        >
            ...
            >
                <Row align="bottom" justify="space-between">
                    <Col>
                        {!collapsed ? <MenuLogo onClose={onClose} /> : ""}
                    </Col>
                    {customise.sidebarCollapseButton && (
                        <Col className={!collapsed ? "hp-pr-0" : "hp-d-flex-full-center"}>
...
// Prop types validation for the Sidebar component
Sidebar.propTypes = {
